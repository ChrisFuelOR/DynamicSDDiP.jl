...
# > "solve_all_children"
# are derived from similar named functions (backward_pass, calculate_bound,
# solve_first_stage_problem, solve_all_children) in the 'SDDP.jl' package by
    ...
    for index in length(scenario_path):-1:1
        ...
        for cut_generation_regime in algo_params.cut_generation_regimes
            ...
            if (model.ext[:iteration] >= cut_generation_regime.iteration_to_start
                ...
                ################################################################
                #NOTE: I did not include the similar node handling from SDDP.jl.
