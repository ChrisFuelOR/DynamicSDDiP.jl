...
function model_definition(fixed_x::Float64, problem_params::DynamicSDDiP.ProblemParams)
    ...
    model = SDDP.LinearPolicyGraph(
        ...
        # State variables
        JuMP.@variable(subproblem, 0.0 <= x <= 3.0, SDDP.State, Int, initial_value = 0)
        ...
        if t == 1
            ...
            # Local variables
            JuMP.@variable(subproblem, 0.0 <= y <= 3.0, Int)
            # JuMP.set_integer(x[2])

            # Constraints
            x = subproblem[:x]
            JuMP.@constraint(subproblem, con, 4/3*y >= x.in)

            # Stage objective
            SDDP.@stageobjective(subproblem, y)
...
model_starter(3.0, :midpoint, DynamicSDDiP.ConvexHullCopy(), DynamicSDDiP.IntegerRelax(), DynamicSDDiP.PrimalObj(), DynamicSDDiP.Unbounded_Opt_Bound(strict=true, user_dual_multiplier_bound=10.0))
