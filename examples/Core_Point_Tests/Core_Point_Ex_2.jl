...
function algo_config(
    ...
    elseif core_point_approach == :eps
        dual_normalization_regime = DynamicSDDiP.Core_Epsilon(copy_regime=copy_regime, integer_regime=integer_regime, unbounded_regime=unbounded_regime, improvement_regime=improvement_regime, normalize_direction=false)
    elseif core_point_approach == :conv
        dual_normalization_regime = DynamicSDDiP.Core_Conv(lambda=0.9, copy_regime=copy_regime, integer_regime=integer_regime, unbounded_regime=unbounded_regime, improvement_regime=improvement_regime, normalize_direction=false)
    end
...
function model_definition(fixed_x::Float64, problem_params::DynamicSDDiP.ProblemParams)
    ...
    model = SDDP.LinearPolicyGraph(
        ...
        if t == 1
            ...
            # Constraints
            x = subproblem[:x]
            JuMP.@constraint(subproblem, con1, y >= 1/4 * x.in)
            JuMP.@constraint(subproblem, con2, y >= 1/2 * x.in - 1)
            JuMP.@constraint(subproblem, con3, y >= x.in - 4)
            JuMP.@constraint(subproblem, con4, u >= 1/8 * x.in + 1/8)
