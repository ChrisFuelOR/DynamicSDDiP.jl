...
function model_definition(problem_params::DynamicSDDiP.ProblemParams, scenario_tree)
    ...
    model = SDDP.LinearPolicyGraph(
        ...
        # Add stage objective
        SDDP.@stageobjective(subproblem, 1/1000 * sum(f[i] * y[i] for i in 1:number_of_facilities) + 1/1000 * sum(d[j][i] * x[i,j] for j in 1:number_of_customers for i in 1:number_of_facilities))
